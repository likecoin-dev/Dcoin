...
/** An output of a transaction.  It contains the public key that the next input Результат транзакции.  Он содержит открытый ключ, следующий
 * must be able to sign with to claim it.                                       входные данные должны быть в состоянии подписать его требования.
 */
class CTxOut
{
public:
...
/** The basic transaction that is broadcasted on the network and contained in   Основные сделки, транслируемые в сети, и содержится в блоках.
 * blocks.  A transaction can contain multiple inputs and outputs.              Сделки могут содержать несколько входов и выходов.
 */
class CTransaction
{
public:
 ...
 * - The higher bits encode N, the number of non-zero bytes in the following bitvector.     Высшие биты кодируют N, количество ненулевых байтов в следующем битвекторе.
...
/** Nodes collect new transactions into a block, hash them into a hash tree,        Узлы собирают новые транзакции в блок, их хеши в хеш-дерево
 * and scan through nonce values to make the block's hash satisfy proof-of-work     и сканируют через nonce значения хеша в блоках удовлетворяющие proof-of-work.
 * requirements.  When they solve the proof-of-work, they broadcast the block       Когда они решить proof-of-work, они передаются в блок для всех,
 * to everyone and the block is added to the block chain.  The first transaction    и блок добавляется в block chain. Первая транзакция в блоке это одна специальная,
 * in the block is a special one that creates a new coin owned by the creator       создающая новую монету, принадлежащую создателю блока.
 * of the block.
 */
class CBlockHeader
{
