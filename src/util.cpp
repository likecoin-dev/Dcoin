                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No Dcoin.conf file is OK
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it)
    {
        ...
        //              Не перезаписывать существующие параметры, параметры командной строки переопределены bitcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "dbcoind.pid"));
