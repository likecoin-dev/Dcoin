...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight+1, nFees);   ////////// транзакция с новыми монетами блока //////////
        ...
        pblock->Dbase[0]    = pindexPrev->Dbase[0] + GetBlockValue(pindexPrev->nHeight+1, nFees);       ////////// новое //////////
        pblock->Dbase[1]    = pindexPrev->Dbase[1] + GetBlockValue(pindexPrev->nHeight+1, nFees);       ////////// новое //////////
        pblock->coinDs[0]   = pindexPrev->coinDs[0] + GetBlockValue(pindexPrev->nHeight+1, nFees);      ////////// новое //////////
        pblock->coinDs[1]   = pindexPrev->coinDs[1] + GetBlockValue(pindexPrev->nHeight+1, nFees);      ////////// новое //////////
...
void FormatHashBuffers(CBlock* pblock, char* pmidstate, char* pdata, char* phash1)
{
    ...
    struct
    {
        struct unnamed2
        {
            ...
            unsigned int nBits;
            unsigned int nNonce;
//            uint64 Dbase[2];          ////////// новое //////////  ошибка на 575 строчке
...
//    tmp.block.Dbase[0]       = pblock->Dbase[0];       ////////// новое //////////  ошибка на 575 строчке
//    tmp.block.Dbase[1]       = pblock->Dbase[1];       ////////// новое //////////
//    tmp.block.coinDs[0]      = pblock->coinDs[0];      ////////// новое //////////
//    tmp.block.coinDs[1]      = pblock->coinDs[1];      ////////// новое //////////
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution                                                             Найденное решение
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("dbcoinMiner : generated block is stale");
        ...
        // Process this block the same as if we had received it from another node   Обработать этот блок так же, как если бы мы получили его от другого узла
        CValidationState state;
        if (!ProcessBlock(state, NULL, pblock))
            return error("DcoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { while (true) {
        ...
        printf("Running dbcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
