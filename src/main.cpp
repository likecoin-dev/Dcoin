...
int64 CTransaction::nMinTxFee = 10000;  // Override(дублирование) with -mintxfee
...
int64 GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / Params().SubsidyHalvingInterval());     // Присваивание с побитовым сдвигом вправо
        ...
        vector<CInv> vInvWait;
        {
            ...
            BOOST_FOREACH(const CInv& inv, pto->vInventoryToSend)
            {
                ...
                if (inv.type == MSG_TX && !fSendTrickle)
                {
                    ...
                    // always trickle our own transactions      (всегда сочиться наших собственных операций)
                    if (!fTrickleWait)
                    {
                        CWalletTx wtx;
