 ...
 *
 * Will automatically close the file when it goes out of scope if not null.         Автоматически закроет файл когда он выходит из области видимости, если не нулевой.
 * If you're returning the file pointer, return file.release().                     Если вы возвращаете указатель файла, возвращайте file.release().
 * If you need to close the file early, use file.fclose() instead of fclose(file).  Если вам нужно закрыть файл рано, используйте file.fclose() вместо fclose(file)
 */
class CAutoFile
{
protected:
